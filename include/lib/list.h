...
typedef struct list_node_t {
    struct list_node_t* prev; /* Previous node in doubly-linked list */
    struct list_node_t* next; /* Next node in doubly-linked list     */
} list_node_t;
 ...
 */
static inline list_node_t* list_get_front(list_node_t* head) {
    return head->next;
}
 ...
 */
static inline int list_is_empty(const list_node_t* head) {
 ...
 */
static inline int list_is_attached(const list_node_t* node) {
 ...
 * @head: The head of the list.
 */
static inline void list_add_to_head(list_node_t* node, list_node_t* head) {
    head->next->prev = node;
    node->next       = head->next;
    node->prev       = head;
    head->next       = node;
 ...
 * @head: The head of the list.
 */
static inline void list_add_to_tail(list_node_t* node, list_node_t* head) {
    head->prev->next = node;
    node->prev       = head->prev;
    node->next       = head;
    head->prev       = node;
