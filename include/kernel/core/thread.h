...
typedef struct thread_ctrl_blk_s {
    ...
    /* IPC fields            */
    ipc_msg_s*       tx_msg;          /* Pending send buffer             */
    ...
    reply_s*         reply;           /* Reply object                    */

    /* Capability fields     */
    capability_s     root;            /* CSpace root CNode capability    */
 ...
 * @root: The thread's root capability, assigned before the thread
 *        becomes schedulable.
...
/**
 * thread_init - Initializes @thread to a blocked state.
...
/**
 * thread_config - Configures the retyped TCB @thread, allocating its
 ...
 * @sp: The thread's initial user-mode stack pointer.
 * @cr3: The thread's address space page directory.
 * @root: The thread's root capability.
...
/**
 * thread_resume - Makes the configured TCB @thread schedulable.
