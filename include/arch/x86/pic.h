...
/**
 * pic_remap - Remaps the master and slave PICs to offsets @offset1 and
 *             @offset2, respectively. After remapping, IRQs 0-7 map to
 *             @offset1 to @offset1 + 7 and IRQs 8-15 to @offset2 to
 *             @offset2 + 7. Unmask all IRQ lines.
...
/**
 * pic_disable - Disables the 8259 PIC. Unmasks all IRQ lines.
 */
void pic_disable(void);

/**
 * irq_set_mask - Masks @irq.
 * @irq: The IRQ number to mask.
 */
void irq_set_mask(u8 irq);

/**
 * irq_clear_mask - Unmasks @irq.
 * @irq: The IRQ number to unmask.
 */
void irq_clear_mask(u8 irq);

/**
 * pic_get_irr - Returns the combined IRQ request register from both
 *               PICs.
 * Returns: The combined IRR value; bits 0-7 master, bits 8-15 slave.
 */
u16 pic_get_irr(void);

/**
 * pic_get_isr - Returns the combined IRQ in-service register from
 *               both PICs.
 * Returns: The combined ISR value; bits 0-7 master, bits 8-15 slave.
 */
u16 pic_get_isr(void);

/**
 * pic_init - Initializes 8259 Programmable Interrupt Controller (PIC).
 *            Remaps the master and slave PICs to offsets @offset1 and
