...
} __packed pg_dir_entry_s;

typedef struct pg_table_entry_s {
    u8          present  : 1;  /* Present flag                */
    u8          rw       : 1;  /* Writable flag               */
    u8          user     : 1;  /* User access flag            */
    u8          pwt      : 1;  /* Write through flag          */
...
static inline pg_dir_s* get_pg_dir(void) {
    return (pg_dir_s*)PG_DIR;
}

static inline pg_table_s* get_pg_table(u32 pg_dir_idx) {
    return (pg_table_s*)PG_TABLE(pg_dir_idx);
}

static inline pg_dir_entry_s* get_pg_dir_entry(u32 pg_dir_idx) {
    return &get_pg_dir()->entries[pg_dir_idx];
}

static inline pg_table_entry_s* get_pg_table_entry(u32 pg_dir_idx, u32 pg_table_idx) {
...
static inline void set_pg_table_entry(pg_table_entry_s* entry, const phys_addr_t paddr, const u32 flags) {
    ...
    entry->user    = (flags & PG_USER_FLAG) ? 1 : 0;
    entry->global  = (flags & PG_GLOBAL_FLAG) ? 1 : 0;
    entry->paddr   = paddr >> 12;
}
...
static void __init alloc_pg_table(virt_addr_t vaddr) {
    ...
    set_pg_dir_entry(pg_dir_entry, paddr, PG_RW_FLAG);
...
void vmm_pg_fault_handler(const int_frm_s* frm) {
    if (likely(!(frm->err & 1))) {
        ...
        if (likely(cr2 >= (virt_addr_t)skheap && cr2 < (virt_addr_t)ekheap)) {
            vmm_demand_map(cr2 & ~(PG_SZ - 1u), PG_RW_FLAG);
            return;
        }
    }

    PANIC("Error: Failed to recover from exception");
}
...
void vmm_set_pg_flags(virt_addr_t vaddr, u32 flags) {
    ...
    pg_table_entry_s* pg_table_entry = get_pg_table_entry(pg_dir_idx, pg_table_idx);
    if (unlikely(!pg_table_entry->present))
        return;

    pg_table_entry->rw     = (flags & PG_RW_FLAG) ? 1 : 0;
    pg_table_entry->user   = (flags & PG_USER_FLAG) ? 1 : 0;
    pg_table_entry->global = (flags & PG_GLOBAL_FLAG) ? 1 : 0;
...
phys_addr_t vmm_create_aspace(void) {
    ...
    vmm_map_pg(paddr, VMM_MMU_SCRATCH, PG_RW_FLAG);
    ...
    set_pg_dir_entry(&new->entries[PG_DIR_IDX], paddr, PG_RW_FLAG);
...
void vmm_destroy_aspace(phys_addr_t cr3) {
    ...
    vmm_map_pg(cr3, VMM_MMU_SCRATCH, PG_RW_FLAG);
    pg_dir_s* kernel_pg_dir = (pg_dir_s*)VMM_MMU_SCRATCH;
    for (u32 pg_dir_idx = 0; pg_dir_idx < HIGHER_HALF_IDX; pg_dir_idx++) {
        ...
        pg_table_s* pg_table = (pg_table_s*)VMM_MMU_SCRATCH;
        ...
        pmm_free_frm(paddr);
        vmm_map_pg(cr3, VMM_MMU_SCRATCH, PG_RW_FLAG);
        kernel_pg_dir = (pg_dir_s*)VMM_MMU_SCRATCH;
    }
