...
static void check_symlinks(void) {
    ...
    check(do_unlink("/alias") == E_OK && do_unlink("/loopa") == E_OK &&
...
static void check_mounts(void) {
    ...
    check(do_mount("none", "/mnt", "ramfs", 0) == E_OK,              "mount");
    check(do_mount("none", "/mnt/absent", "ramfs", 0) == -(i32)E_NOENT, "mount on missing dir fails");
...
static void check_truncate(void) {
    ...
    check(fd >= 0 && fd_read(0, fd, buffer, 200) == 100 && memcmp(buffer, "hel", 3) == 0 &&
          buffer[3] == 0 && buffer[99] == 0,                                         "truncate zero fill");
    fd_close(0, fd);
...
static void check_bigfile(void) {
    ...
    check(fd_read(0, fd, buffer, sizeof(buffer)) == (i32)sizeof(buffer) &&
...
void vfs_selftest(void) {
    ...
    const dcache_stats_s* stats = dcache_stats();
    vfs_log("[VFS] Self-test: %u passed, %u failed\n", passed, failed);
    vfs_log("[VFS] dcache: %u hits, %u misses, %u allocated, %u pruned\n",
            stats->hits, stats->misses, stats->allocated, stats->pruned);
    vfs_log("[VFS] heap: %u of %u pages free\n", heap_pages_free(), VFS_HEAP_PG);
}
