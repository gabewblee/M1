...
static i32 ata_data_wait(const ata_channel_s* channel) {
...
static i32 ata_done_wait(const ata_channel_s* channel) {
    for (u32 spins = 0; spins < 1000000u; spins++) {
        u8 status = ata_read_status(channel);
        if (status & ATA_STATUS_REG_BSY_BIT)
            continue;
...
static i32 ata_issue_lba28(const ata_drv_s* drv, u64 lba, u8 cnt, u8 cmd) {
    const ata_channel_s* channel = drv->channel;
...
static i32 ata_issue_lba48(const ata_drv_s* drv, u64 lba, u16 cnt, u8 cmd) {
    const ata_channel_s* channel = drv->channel;
...
static i32 ata_read_lba28(const ata_drv_s* drv, u64 lba, u8 cnt, void* buf) {
    const ata_channel_s* channel = drv->channel;
    u16* words = (u16*)buf;

    ata_issue_lba28(drv, lba, cnt, ATA_READ28_CMD);
    const u32 total = (!cnt) ? 256u : (u32)cnt;

    for (u32 sec = 0; sec < total; sec++) {
        i32 ret = sys_irq_wait(channel->irq);
        if (ret != E_OK)
            return ret;
...
static i32 ata_write_lba28(const ata_drv_s* drv, u64 lba, u8 cnt, const void* buf) {
    const ata_channel_s* channel = drv->channel;
    const u16* words = (const u16*)buf;
    ...
    if (sys_irq_wait(channel->irq) != E_OK)
        return -(i32)E_AGAIN;

    return ata_done_wait(channel);
}

static i32 ata_read_lba48(const ata_drv_s* drv, u64 lba, u16 cnt, void* buf) {
    const ata_channel_s* channel = drv->channel;
    u16* words = (u16*)buf;

    ata_issue_lba48(drv, lba, cnt, ATA_READ48_CMD);
    const u32 total = (!cnt) ? 65536u : (u32)cnt;
...
static i32 ata_write_lba48(const ata_drv_s* drv, u64 lba, u16 cnt, const void* buf) {
    const ata_channel_s* channel = drv->channel;
    const u16* words = (const u16*)buf;
