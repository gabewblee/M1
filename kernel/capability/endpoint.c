...
static i32 transfer(thread_ctrl_blk_s* dst, thread_ctrl_blk_s* src, u32 badge, msg_info_t mi) {
    i32 ret = copy_between_user(dst->cr3, dst->rx_packet, src->cr3, src->tx_packet, IPC_PACKET_SZ);
    ...
    /* Transfer capability from src to dst */
    if (src->tx_depth && dst->rx_depth) {
        i32 xfer = capability_transfer(
            &src->cspace_root, src->tx_cptr, src->tx_depth,
            &dst->cspace_root, dst->rx_cptr, dst->rx_depth
        );
...
static i32 notification_wait(notification_s* nt, u32* signals) {
    thread_ctrl_blk_s* cur = thread_self();
...
i32 ipc_send(const capability_s* root,
    ...
    thread_ctrl_blk_s* cur = thread_self();
    cur->tx_packet         = packet;
    cur->ipc_badge         = slot->capability.badge;
    cur->ipc_msg_info      = mi;
    cur->cspace_root       = *root;
...
i32 ipc_signal(const capability_s* root, u32 ntfn_cptr) {
    cte_s* slot = resolve(root, ntfn_cptr, CAPABILITY_TYPE_NOTIFICATION, CAPABILITY_RIGHT_WRITE);
    if (unlikely(!slot))
        return -(i32)E_PERM;

    u32 badge = slot->capability.badge;
    notification_signal((notification_s*)slot->capability.obj, badge ? badge : 1u);
    return E_OK;
}

i32 ipc_wait(const capability_s* root, u32 ntfn_cptr, u32* badge) {
    cte_s* slot = resolve(root, ntfn_cptr, CAPABILITY_TYPE_NOTIFICATION, CAPABILITY_RIGHT_READ);
    if (unlikely(!slot))
        return -(i32)E_PERM;

    u32 signals = 0;
    i32 ret = notification_wait((notification_s*)slot->capability.obj, &signals);
    if (badge)
        *badge = signals;
