...
static i32 invoke_retype(const capability_s* root, cte_s* untyped, const u32* args) {
    ...
    u32 slots = 1u << cnode->capability.radix, idx = args[4], cnt = args[5];
    if (unlikely(idx >= slots || cnt > slots - idx))
        return -(i32)E_INVAL;

    cte_s* dst = &((cte_s*)cnode->capability.obj)[idx];
    return capability_retype(untyped, (capability_type_e)args[0], args[1], dst, cnt);
}
...
static i32 invoke_tcb_configure(const capability_s* root,
    ...
    if (unlikely(thread->capability.type != CAPABILITY_TYPE_TCB))
        return -(i32)E_INVAL;
    ...
    return thread_config(
        (thread_ctrl_blk_s*)thread->capability.obj,
        cur->task,
        (virt_addr_t)args[4],
        (virt_addr_t)args[5],
...
static i32 invoke_irq_get(const capability_s* root, cte_s* ctrl, const u32* args) {
    if (unlikely(ctrl->capability.type != CAPABILITY_TYPE_IRQ_CTRL))
        return -(i32)E_INVAL;
...
static i32 invoke_irq_set_ntfn(const capability_s* root, cte_s* handler, const u32* args) {
    ...
    if (unlikely(!ntfn || ntfn->capability.type != CAPABILITY_TYPE_NTFN))
        return -(i32)E_INVAL;

    return irq_bind_ntfn((u8)handler->capability.badge, (ntfn_s*)ntfn->capability.obj);
}
...
static i32 invoke_map_pg(const capability_s* root, cte_s* frame, const u32* args) {
    ...
    return vmm_map_frm_in(
        (phys_addr_t)vspace->capability.obj,
        __pa(frame->capability.obj),
        vaddr,
        flags
    );
}
...
static i32 invoke_map_pg_table(const capability_s* root, cte_s* table, const u32* args) {
    ...
    return vmm_map_pg_table_in(
        (phys_addr_t)vspace->capability.obj,
        (phys_addr_t)table->capability.obj,
        vaddr,
        PG_USER_FLAG
    );
}
...
i32 capability_invoke(thread_ctrl_blk_s* cur, cte_s* slot, u32 label, const u32* args, u32 argc) {
    ...
    switch (label) {
        ...
        case CAPABILITY_LABEL_CNODE_CP:
        case CAPABILITY_LABEL_CNODE_MINT:
        case CAPABILITY_LABEL_CNODE_MV: {
            ...
            if (label == CAPABILITY_LABEL_CNODE_CP)
        ...
        case CAPABILITY_LABEL_CNODE_DEL:
        case CAPABILITY_LABEL_CNODE_RVK: {
            if (unlikely(slot->capability.type != CAPABILITY_TYPE_CNODE))
                return -(i32)E_INVAL;
            ...
            return label == CAPABILITY_LABEL_CNODE_DEL ? capability_del(target) : capability_rvk(target);
        }
        ...
        case CAPABILITY_LABEL_TCB_RESUME:
            ...
            thread_resume((thread_ctrl_blk_s*)slot->capability.obj);
            return E_OK;
        case CAPABILITY_LABEL_MAP_PG:
            return invoke_map_pg(root, slot, args);
        case CAPABILITY_LABEL_UNMAP_PG:
            return invoke_unmap_pg(root, slot, args);
        case CAPABILITY_LABEL_MAP_PG_TABLE:
